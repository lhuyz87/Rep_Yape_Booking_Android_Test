...
//        System.out.println(" horaaaa"  + fecha);
	...
	public void localizarElementoPorTexto(AppiumDriver driver, WebElement elemento, String texto) {
		
		System.out.println(elemento.getText() + " vs " + texto);
		
		while(elemento.getText().compareTo(texto)!=0) {
...
//	                i = numberOfTimes;
	  ...
	  public static void scrollPequeño(WebDriver driver) {
		  
		  int startX = driver.manage().window().getSize().width / 2;
		  int startY = (int) (driver.manage().window().getSize().height * 0.8); // 80% desde la parte superior
		  int endY = (int) (driver.manage().window().getSize().height * 0.2);   // 20% desde la parte superior

		  
		  
		  
	        new TouchAction((PerformsTouchActions) driver).press(PointOption.point(startX, startY))
	           .waitAction(WaitOptions.waitOptions(Duration.ofMillis(500))) // Opcional: Pausa durante 500 ms
